...
function getNodePos(n, addNext) {
  ...
  if (addNext && n.hasChildNodes()) {
    ...
    if (lastChild.hasChildNodes()){
      ...
      pos.next.ch += lastChild.textContent.length;
    }
  }
  ...
  attachInfo(json, node) {
    ...
    if (nodePresentation) {
      if (nodePresentation.lineNumber) {
        json.pos.pmml = getNodePos(nodePresentation, true);
      }
    ...
    if (presentation) {
      if (!json.pos.pmml && presentation.lineNumber) {
        json.pos.pmml = getNodePos(presentation);
      }
      json.presentation = this.constructor.serialize(presentation, true);
    }
    return json;
  }
