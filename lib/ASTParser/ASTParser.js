...
module.exports = class ASTParser {
  ...
  attachInfo(json, node) {
    ...
    json.pos = {
      cmml: {
        line: node.lineNumber - 1, // CodeMirror starts counting at 1
        ch: node.columnNumber
      }
    };
    const presentation = json.presentation;
    const nodePresentation = json.nodePresentation;
    if (nodePresentation) {
      if (nodePresentation.lineNumber) {
        json.pos.pmml = {
          line: nodePresentation.lineNumber - 1, // CodeMirror starts counting at 1
          ch: nodePresentation.columnNumber
        };
      }
      json.nodePresentation = this.constructor.serialize(nodePresentation);
    }
    if (presentation) {
      if (!json.pos.pmml && presentation.lineNumber) {
        json.pos.pmml = {
          line: presentation.lineNumber - 1, // CodeMirror starts counting at 1
          ch: presentation.columnNumber
        };
      }
      json.presentation = this.constructor.serialize(presentation);
    }
    return json;
  }
  ...
  extractChilds(element) {
    ...
    return this.attachInfo({
      nodePresentation: leafPresentation,
      name,
      id: elementId,
