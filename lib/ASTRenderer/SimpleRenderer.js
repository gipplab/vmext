...
module.exports = class SimpleRenderer {
  ...
  renderSVGwithMarginStylesAndScript(window) {
    const svg = window.d3.select('body')
      ...
      .append('svg')
      .attr('width', '100%')
      .attr('height', `${this.defaults.height + this.dimensions.margin.top + this.dimensions.margin.bottom}`)
  ...
  render({ data, renderFormula }) {
    return this.marginedSVG.then(({ svg, window }) => {
      ...
      const ast = svg.append('g')
        .attr('transform', () => {
          return renderFormula ? `translate(0,${100})` : 'translate(0,0)';
        })
        .attr('class', 'ast');
      ...
      // render legend
      const legend = ast.append('g')
        .attr('class', 'legend')
        .attr('transform', `translate(${this.defaults.width - this.dimensions.margin.right - 150}, 0)`);
      legend.append('rect')
        .attr('width', 150)
        .attr('height', this.defaults.minNodeWidth)
        .attr('rx', 7)
        .attr('ry', 7);
      legend.append('text')
      ...
      const node = ast.selectAll('.node')
        ...
        .attr('data-xref', d => d.data.presentation_id);
  ...
  viewBox() {
    return `0 0 ${this.defaults.width + this.dimensions.margin.left + this.dimensions.margin.right} 
    ${this.defaults.height + this.dimensions.margin.top + this.dimensions.margin.bottom}`;
  }
};
