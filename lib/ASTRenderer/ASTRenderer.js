...
module.exports = class ASTRenderer {
  ...
  constructor() {
    ...
    this.marginedSVG = this.window.then(window => this.renderSVGwithMarginStylesAndScript(window));
  }
  ...
  renderSVGwithMarginStylesAndScript(window) {
    ...
    const scripts = svg.append('script').html(SVGscripts).attr('type', 'text/javascript');
  ...
  renderAST({ data, renderFormula: renderFormula = false }) {
    return this.marginedSVG.then((svg) => {
        ...
        mathJaxRenderings.push(
          new Promise((resolve, reject) => {
            MathJaxRenderer.renderMML(data.presentation, (err, entireFormula) => {
              if (err) reject(err);
              const svgSize = ASTRenderer.getSVGsize(entireFormula);
              this.window.d3.select('.header')
              .append('g')
      ...
      node.each(function(d) {
        mathJaxRenderings.push(new Promise((resolve, reject) => {
          MathJaxRenderer.renderMML(d.data.nodePresentation, (mathjaxErr, mmlSVG) => {
            ...
            const svgSize = ASTRenderer.getSVGsize(mmlSVG);
      ...
      return Promise.all(mathJaxRenderings).then(() => {
        return this.window.d3.select('.container').html();
      });
    });
  }
