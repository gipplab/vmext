...
module.exports = class GraphRenderer {
  ...
  /**
   * Just render the syntax tree of the first formula that this instance was initialized.
  ...
  renderMathML(cy) {
    return new Promise((resolve, reject) => {
      Promise.all(cy.nodes().map((el) => {
        let mathml = (el.hasClass('collapse')) ? el.data('subtreePresentation') : el.data('nodePresentation');
        mathml = (mathml == null) ? '<math></math>' : mathml;
        return MathJaxRenderer.renderMML(mathml)
        .then((nodeSVG) => {
          el.data('nodeSVG', `data:image/svg+xml;utf8,${querystring.escape(nodeSVG)}`);
          el.removeData('nodePresentation');
          if (el.data('subtreePresentation') != null) {
            return MathJaxRenderer.renderMML(el.data('subtreePresentation'))
            .then((subtreeSVG) => {
              el.data('subtreeSVG', `data:image/svg+xml;utf8,${querystring.escape(subtreeSVG)}`);
