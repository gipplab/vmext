...
module.exports = class GraphRenderer {
  ...
  collapseSimilarMatches(cy) {
    cy.edges('[matchType="similar"]').forEach((edge, i) => {
      ...
      if (nodeA.outgoers('[type="hierarchy"]').length > 0) {
        nodeA.addClass('collapse');
      }
      if (nodeB.outgoers('[type="hierarchy"]').length > 0) {
        nodeB.addClass('collapse');
      }
    ...
    for (let i = 0; i < leaves.length; i++) {
      ...
      while (noMatchesFound && current) {
        ...
        if (noMatchesFound) {
          ...
          if (previous) {
            collapseNodeIds.delete(previous[0].id());
          }
  ...
  addNodes(tree, options) {
    return new Promise((resolve) => {
      ...
      while (queue.length > 0) {
        ...
        const classes = [];
        if (options.source) {
          classes.push(`source-${options.source}`);
        }
        if (current.name === 'ambiguous') {
