...
module.exports = class GraphRenderer {
  ...
  renderMathML(cy) {
    return new Promise((resolve, reject) => {
      ...
      cy.nodes().forEach((el) => {
        // find appropriate mathml text, depending on whether this node should be collapsed
  ...
  collapseSimilarMatches(cy) {
    cy.edges('[matchType="similar"]').forEach((edge, i) => {
      ...
      if (!nodeA.outgoers('[type="hierarchy"]').length) nodeA.addClass('collapse');

      nodeB
        .move({ parent: compoundNode.id() })
        .addClass('match match-similar');
      if (!nodeB.outgoers('[type="hierarchy"]').length) nodeB.addClass('collapse');
    });
  }
